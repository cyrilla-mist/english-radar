...
if (!Array.isArray(signals) || signals.length !== 62) errors.push(`Expected 62 signals, found ${signals && signals.length}.`);
...
if (!Array.isArray(quizzes) || quizzes.length !== 124) errors.push(`Expected 124 quizzes, found ${quizzes && quizzes.length}.`);
...
if (errors.length) { console.error('English Radar data validation failed.'); errors.forEach((error) => console.error(`- ${error}`)); process.exitCode = 1; } else { console.log('English Radar data validation passed.'); console.log('62 signals'); console.log('124 quizzes'); console.log('All signal and quiz IDs are valid.'); }
