...
if (!Array.isArray(signals) || signals.length !== 66) errors.push(`Expected 66 signals, found ${signals && signals.length}.`);
...
if (!Array.isArray(quizzes) || quizzes.length !== 132) errors.push(`Expected 132 quizzes, found ${quizzes && quizzes.length}.`);
...
if (errors.length) { console.error('English Radar data validation failed.'); errors.forEach((error) => console.error(`- ${error}`)); process.exitCode = 1; } else { console.log('English Radar data validation passed.'); console.log('66 signals'); console.log('132 quizzes'); console.log('All signal and quiz IDs are valid.'); }
