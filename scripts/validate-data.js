...
if (!Array.isArray(signals) || signals.length !== 68) errors.push(`Expected 68 signals, found ${signals && signals.length}.`);
...
if (!Array.isArray(quizzes) || quizzes.length !== 136) errors.push(`Expected 136 quizzes, found ${quizzes && quizzes.length}.`);
...
if (errors.length) { console.error('English Radar data validation failed.'); errors.forEach((error) => console.error(`- ${error}`)); process.exitCode = 1; } else { console.log('English Radar data validation passed.'); console.log('68 signals'); console.log('136 quizzes'); console.log('All signal and quiz IDs are valid.'); }
