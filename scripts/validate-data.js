...
if (!Array.isArray(signals) || signals.length !== 69) errors.push(`Expected 69 signals, found ${signals && signals.length}.`);
...
if (!Array.isArray(quizzes) || quizzes.length !== 138) errors.push(`Expected 138 quizzes, found ${quizzes && quizzes.length}.`);
...
if (errors.length) { console.error('English Radar data validation failed.'); errors.forEach((error) => console.error(`- ${error}`)); process.exitCode = 1; } else { console.log('English Radar data validation passed.'); console.log('69 signals'); console.log('138 quizzes'); console.log('All signal and quiz IDs are valid.'); }
