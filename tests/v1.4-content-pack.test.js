...
assert.equal(content.getActiveLearningSignals().length, 69);
assert.equal(registry.getStaticQuizzes().length, 226);
...
assert.equal(content.getActiveLearningSignals().length, 79);
...
assert.equal(content.getActiveLearningSignals().length, 103);
...
content.invalidate();
assert.equal(content.getActiveLearningSignals().length, 93);
...
for (const page of ['index.html', 'learn.html', 'dictionary.html', 'inbox.html', 'quiz.html', 'me.html']) {
  ...
  assert(/v0\.1\.0/i.test(html), `${page} must expose current V0.1.0 metadata`);
}
assert(/Sideglance Radar v0\.1\.0 signal not found\./.test(read('404.html')));
...
assert(read('me.html').includes('./js/me.js?v=1.8.3'));
