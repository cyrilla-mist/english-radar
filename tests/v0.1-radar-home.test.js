...
assert.deepEqual(labels, ["TODAY'S FOCUS", 'DAILY MIX', 'ON THE RADAR', 'WORTH ANOTHER LOOK', 'CONNECTION', 'CAPTURE']);
...
assert.match(home, /SideglanceSignalResolver/);
...
assertCue(contrast, 'CONTRAST', 'Contrasts with "effect".');
...
assertCue(unseen, 'NEW', '');
const fallback = resolveCue(null, [], {});
assertCue(fallback, 'RADAR PICK', '');
...
assert.match(home, /daily-mix-context/);
...
assert.equal(resolveIdentity({ id: 'a', category: 'Community Discourse', platforms: ['Reddit'], tone: ['Casual'] }), 'Reddit · Casual · Community Discourse');
assert.equal(resolveIdentity({ id: 'b', signalIdentity: { type: 'Developer', usageContext: 'code review', developerContext: 'GitHub', tone: 'direct', relationshipHint: 'review language' } }), 'code review · GitHub · direct');
assert.equal(resolveIdentity({ id: 'b2', category: 'Product', productContext: 'Roadmap language' }), 'Roadmap language · Product');
