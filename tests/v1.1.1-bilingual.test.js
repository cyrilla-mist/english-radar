...
assert.equal(coreSignals.length, 66);
assert(coreSignals.every((signal) => signal.useWhenZh && signal.avoidWhenZh));
assert.equal(new Set(coreSignals.map((signal) => signal.useWhenZh)).size, 66);
assert.equal(new Set(coreSignals.map((signal) => signal.avoidWhenZh)).size, 66);
assert.equal(staticQuizzes.length, 132);
