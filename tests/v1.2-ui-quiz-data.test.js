...
assert.equal(core.length, 124, 'Core Quiz runtime count must remain 124');
assert.equal(ui.length, 20, 'UI Quiz count must be exactly 20');
assert.equal(registry.getStaticQuizzes().length, 144, 'Static Quiz registry should contain 144 questions');
