...
assert.equal(core.length, 30, 'Core Quiz source must remain unchanged');
assert.equal(ui.length, 20, 'UI Quiz count must be exactly 20');
assert.equal(registry.getStaticQuizzes().length, core.length + 20, 'Static Quiz registry should add exactly 20 UI questions');
