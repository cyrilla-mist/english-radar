...
assert.equal(core.length, 132, 'Core Quiz runtime count must remain 132');
assert.equal(ui.length, 20, 'UI Quiz count must be exactly 20');
assert.equal(registry.getStaticQuizzes().length, 152, 'Static Quiz registry should contain 152 questions');
