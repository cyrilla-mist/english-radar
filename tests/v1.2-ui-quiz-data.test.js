...
assert.equal(core.length, 138, 'Core Quiz runtime count must remain 138');
assert.equal(ui.length, 20, 'UI Quiz count must be exactly 20');
assert.equal(registry.getStaticQuizzes().length, 158, 'Static Quiz registry should contain 158 questions');
