...
assert.equal(core.length, 136, 'Core Quiz runtime count must remain 136');
assert.equal(ui.length, 20, 'UI Quiz count must be exactly 20');
assert.equal(registry.getStaticQuizzes().length, 156, 'Static Quiz registry should contain 156 questions');
