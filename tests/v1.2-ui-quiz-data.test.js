...
assert.equal(core.length, 122, 'Core Quiz runtime count must remain 122');
assert.equal(ui.length, 20, 'UI Quiz count must be exactly 20');
assert.equal(registry.getStaticQuizzes().length, 142, 'Static Quiz registry should contain 142 questions');
