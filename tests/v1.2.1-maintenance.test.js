...
for (const file of pageFiles) {
  ...
  assert.match(html, /v1\.6\.0/i, `${file} should expose V1.6.0`);
...
assert.equal((mainHtml.match(/ENGLISH RADAR \/ V1\.6\.0/g) || []).length, 4);
const readme = read('README.md');
assert.match(readme, /English Radar v1\.6\.0 is the current development version/);
