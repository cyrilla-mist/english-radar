...
assert.equal((read('archive.html').match(/data-archive-result-count/g) || []).length, 3);
assert(read('archive.html').includes('Search terms or contexts / 搜索表达或语境'));
assert(!read('archive.html').includes('SEARCH THE ARCHIVE</span><small>'));
assert(read('archive.html').includes('data-archive-filter'));
assert(read('css/archive.css').includes('flex-wrap: nowrap'));
assert(read('css/archive.css').includes('overflow-x: auto'));
...
assert(!read('archive-signal.html').includes('SIGNAL RECORD / ARCHIVE MODE'));
assert(read('js/archive-signal.js').includes('signal.status || signal.contentStatus'));
