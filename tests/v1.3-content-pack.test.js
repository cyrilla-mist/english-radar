...
assert.equal(quizzes.getStaticQuizzes().length, 206);
...
assert.equal(content.getActiveLearningSignals().length, 81);
...
for (const page of ['index.html', 'learn.html', 'dictionary.html', 'inbox.html', 'quiz.html', 'me.html', '404.html']) {
  ...
  assert.match(html, /v0\.1\.0/i, `${page} should expose current V0.1.0`);
  assert.doesNotMatch(html, /v1\.2\.1/i, `${page} should not expose stale V1.2.1 metadata`);
}
assert.match(meHtml, /me\.js\?v=1\.8\.3/);
