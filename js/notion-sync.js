(function () {
  ...
  function normalizeSyncPayload(payload) {
    ...
    var normalized = []; records.forEach(function (record) { if (!record || !record.signal) return; var signal = registry && registry.normalizeSignal ? registry.normalizeSignal(record.signal, { sourceType: 'notion-sync' }) : null; if (!signal) return; signal = Object.assign({}, signal, { sourceType: 'notion-sync', sourcePackId: '', status: record.signal.status || 'Imported Candidate' }); normalized.push({ notionPageId: String(record.notionPageId || ''), lastEditedAt: record.lastEditedAt || null, signal: signal }); });
    return { ok: true, schemaVersion: 1, source: 'notion', generatedAt: payload.generatedAt || new Date().toISOString(), nextCursor: payload.nextCursor || null, hasMore: payload.hasMore === true, records: normalized, signals: normalized.map(function (record) { return record.signal; }) };
  }
  async function fetchApprovedSignals(options) {
    ...
    while (records.length < MAX_RECORDS) { var params = new URLSearchParams({ status: 'Approved', limit: String(Math.min(100, MAX_RECORDS - records.length)) }); if (cursor) params.set('cursor', cursor); if (opts.since) params.set('since', opts.since); var response = await requestJson(endpoint('/api/signals') + '?' + params.toString(), { method: 'GET', headers: { Accept: 'application/json' }, signal: opts.signal }); if (!response.ok) return response; var payload = normalizeSyncPayload(response.data); if (!payload.ok) return resultError(payload.error); records = records.concat(payload.records); cursor = payload.nextCursor; pages += 1; if (!cursor || payload.hasMore === false || !payload.records.length || pages >= 10) break; }
    return { ok: true, payload: { schemaVersion: 1, source: 'notion', generatedAt: new Date().toISOString(), nextCursor: cursor, hasMore: !!cursor, records: records.slice(0, MAX_RECORDS), signals: records.slice(0, MAX_RECORDS).map(function (record) { return record.signal; }) } };
  }
  ...
  function previewNotionSync(payload) {
    ...
    var existing = registry.getDictionarySignals(); var accepted = []; var valid = []; var exact = []; var possible = []; var invalid = [];
