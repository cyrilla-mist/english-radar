(function () {
  ...
  function renderCard(group, scope, progress) {
    ...
    var meta = metadata(signal); var context = text(meta.usageContext || meta.context || signal.usageContext || meta.relationshipHint || signal.relationshipHint); var contextHtml = context ? '<p class="archive-card-context">' + escapeHtml(context) + '</p>' : ''; var mastery = masteryLabel(signal, progress);
    return '<article class="archive-card ' + (entry.status === 'available' ? 'is-available' : '') + '"><div class="archive-card-top"><span class="archive-record-type">' + escapeHtml(recordType(signal)) + '</span>' + variants + '</div><h3><a class="archive-link" href="./archive-signal.html?id=' + encodeURIComponent(signal.id) + '">' + escapeHtml(signal.displayTerm || signal.term) + '</a></h3><p class="archive-card-category">' + escapeHtml(signal.category || 'Internet context') + '</p>' + fullFormHtml + availabilityHtml(group, scope) + '<p class="archive-card-meaning">' + escapeHtml(text(signal.meaningEn) || 'Indexed language signal.') + '</p>' + contextHtml + (text(signal.meaningZh) ? '<p class="archive-card-zh">' + escapeHtml(signal.meaningZh) + '</p>' : '') + (chips ? '<div class="archive-card-chips">' + chips + '</div>' : '') + '<div class="archive-card-footer"><span class="archive-mastery">STATUS: ' + escapeHtml(mastery) + '</span><a class="archive-open-link" href="./archive-signal.html?id=' + encodeURIComponent(signal.id) + '">Open Signal →</a></div></article>';
  }
  function boot() {
  ...
  window.EnglishRadarArchive = { recordType: recordType, createIndex: createIndex, normalizeTerm: normalizeTerm, searchIndex: search, categoryMatches: categoryMatches, masteryLabel: masteryLabel };
