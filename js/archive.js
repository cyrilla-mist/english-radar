(function () {
  ...
  function renderCard(group, scope) {
    ...
    return '<article class="archive-card ' + (entry.status === 'available' ? 'is-available' : '') + '"><div class="archive-card-top"><span class="archive-record-type">' + escapeHtml(recordType(signal)) + '</span>' + variants + '</div><h3><a class="archive-link" href="./archive-signal.html?id=' + encodeURIComponent(signal.id) + '">' + escapeHtml(signal.displayTerm || signal.term) + '</a></h3>' + availabilityHtml(group, scope) + '<p class="archive-card-meaning">' + escapeHtml(text(signal.meaningEn) || 'Indexed language signal.') + '</p>' + (text(signal.meaningZh) ? '<p class="archive-card-zh">' + escapeHtml(signal.meaningZh) + '</p>' : '') + (chips ? '<div class="archive-card-chips">' + chips + '</div>' : '') + '</article>';
  }
  function boot() {
