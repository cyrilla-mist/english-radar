(function () {
  ...
  function normalizeSignal(raw, context) {
    ...
    var result = Object.assign({}, raw, { id: id, term: term, displayTerm: String(raw.displayTerm || term.toUpperCase()), speechText: String(raw.speechText || term), pronunciation: String(raw.pronunciation || '—'), category: String(raw.category).trim(), platforms: Array.isArray(raw.platforms) ? raw.platforms.slice() : [], tone: Array.isArray(raw.tone) ? raw.tone.slice() : [], status: String(raw.status || 'Imported'), formality: String(raw.formality || '—'), meaningEn: String(raw.meaningEn || '—'), meaningZh: String(raw.meaningZh).trim(), exampleEn: String(raw.exampleEn).trim(), exampleZh: String(raw.exampleZh || '—'), useWhen: String(raw.useWhen || '—'), avoidWhen: String(raw.avoidWhen || '—'), chineseFeeling: String(raw.chineseFeeling || '—'), contentStatus: ['active', 'archived'].indexOf(raw.contentStatus) !== -1 ? raw.contentStatus : 'active', quizStatus: ['none', 'draft', 'ready'].indexOf(raw.quizStatus) !== -1 ? raw.quizStatus : 'none', sourceType: 'imported', sourcePackId: source.packId || raw.sourcePackId || '', createdAt: raw.createdAt || now, updatedAt: now });
