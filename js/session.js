(function () {
  ...
  var reviewSignalId = params.get('signal') || '';
  var isContextReview = mode === 'review' && sourceParam === 'signal-entry' && !!reviewSignalId;
  ...
  function getLearnDisplayTerm(signal) { if (!signal) return ''; return hasText(signal.term) ? String(signal.term).trim() : displayTerm(signal.displayTerm); }
  ...
  function renderContextReview(signal) {
    if (!refs.contextReview) return;
    refs.contextReview.hidden = !isContextReview;
    if (!isContextReview) return;
    setText(refs.contextExample, '“' + text(signal.exampleEn) + '”'); setText(refs.contextQuestion, 'What does ' + text(signal.term) + ' suggest here?'); setText(refs.contextAnswerEn, signal.useWhen || signal.meaningEn); setText(refs.contextAnswerZh, signal.useWhenZh || signal.meaningZh); refs.contextAnswer.hidden = true; refs.contextReveal.hidden = false; refs.contextReveal.textContent = 'Reveal the idea →'; if (refs.contextReturn) refs.contextReturn.href = './signal-entry.html?id=' + encodeURIComponent(signal.id);
  }
  function renderSignal() {
    ...
    var position = currentIndex + 1; var total = sessionSignals.length; var learnDisplayTerm = getLearnDisplayTerm(signal); var interfaceMode = isInterfaceSignal(signal);
    refs.title.textContent = text(signal.term) + ' — Sideglance Radar'; setText(refs.meta, isLookup ? 'DICTIONARY ENTRY' : (mode === 'review' ? 'Review ' : 'SIGNAL ') + String(position).padStart(2, '0') + ' / ' + total); setText(refs.category, interfaceMode ? 'UI VOCABULARY' : isLookup ? signal.category : mode === 'review' ? 'REVIEW' : signal.category);
    ...
    setText(refs.term, learnDisplayTerm); setText(refs.pronunciation, signal.pronunciation); setTemplateMode(signal); if (interfaceMode) renderInterfaceSignal(signal); else renderStandardSignal(signal); renderContextReview(signal);
  ...
  function configureMode() {
    ...
    if (refs.backLink && refs.backLabel) { refs.backLink.href = isLookup ? './dictionary.html' : isContextReview ? './signal-entry.html?id=' + encodeURIComponent(reviewSignalId) : './index.html'; refs.backLabel.textContent = isLookup ? 'Dictionary' : isContextReview ? 'Signal Entry' : 'Exit'; }
  ...
  function initialise() {
    ...
    if (mode === 'review') { sessionSignals = review ? review.getReviewQueue(signals, progressRecords, new Date()) : []; if (isContextReview) sessionSignals = sessionSignals.filter(function (signal) { return signal.id === reviewSignalId; }); sessionSize = sessionSignals.length; }
  ...
  if (refs.contextReveal) refs.contextReveal.addEventListener('click', function () { if (!refs.contextAnswer) return; refs.contextAnswer.hidden = false; refs.contextReveal.hidden = true; });
